...
library(cummeRbund)
library(xlsx)
cuff <- readCufflinks("~/BS7120/Group_Project/Pipeline_5/Final/CuffDiff_rn4_Controls/")
...
newrows <- c("Cufflinks_Gene_ID","Gene_Symbol","FPKM_Control_1", "FPKM_Control_2", "Log2FC","P_Value")
## Loop to change names
for(i in 1:6) colnames(table)[colnames(table) == oldrows[i]] = newrows[i]
## Export to Excel
write.xlsx(table, "DEG_RN4_Controls.xlsx")
